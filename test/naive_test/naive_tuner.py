...
class NaiveTuner(Tuner):
    ...
    def generate_parameters(self, parameter_id, **kwargs):
        self.cur += 1
        _logger.info(f'generate parameters: {self.cur}')
        return { 'x': self.cur }

    def receive_trial_result(self, parameter_id, parameters, value, **kwargs):
        reward = extract_scalar_reward(value)
        _logger.info(f'receive trial result: {parameter_id}, {parameters}, {reward}')
        _result.write('%d %d\n' % (parameters['x'], reward))
        _result.flush()

    def update_search_space(self, search_space):
        _logger.info(f'update_search_space: {search_space}')
